                ...
                {blogpost &&
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-y-[50px] gap-x-6'>
                        ...
                        {blogpost.map((item, index) => {
                            const { title, decription, button, varient, image, authorePublishData } = item || {}
                            return (
                                <div key={index}>
                                    <div>
                                        <div className='border-[1px]  w-full max-w-full md:max-w-[550px] lg:max-w-[628px]   h-[380px] sm:h-[500px] md:h-[560px]  lg:h-[628px]  border-white relative pb-5 mb-6'>
                                            <Image src={image} alt='image' width={400} height={400} className='absolute top-4 -right-4 w-full max-w-full md:max-w-[550px] lg:max-w-[628px]  h-[380px] sm:h-[500px] md:h-[560px]  lg:h-[628px]  object-cover' />
                                        </div>
                                        <span className='text-base italic font-OpenSans mb-5 block'>{authorePublishData}</span>
